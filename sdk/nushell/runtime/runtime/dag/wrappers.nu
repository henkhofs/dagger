...
export def "file" [path: string]: record -> record {
    let obj = $in
    match (get-object-type $obj) {
        "Directory" => { $obj | directory file $path }
        _ => { error make {msg: "file only works on Directory objects"} }
    }
}

# Get a subdirectory from a directory
export def "directory" [path: string]: record -> record {
    let obj = $in
    match (get-object-type $obj) {
        "Directory" => { $obj | directory directory $path }
        _ => { error make {msg: "directory only works on Directory objects"} }
    }
}
...
export def "exists" [path: string]: record -> bool {
    let obj = $in
    match (get-object-type $obj) {
        "Container" => { $obj | container exists $path }
        "Directory" => { $obj | directory exists $path }
        _ => { error make {msg: "exists works on Container or Directory objects"} }
    }
}

# List files matching pattern (works on Directory)
export def "glob" [pattern: string]: record -> list<string> {
    let obj = $in
    match (get-object-type $obj) {
        "Directory" => { $obj | directory glob $pattern }
        _ => { error make {msg: "glob only works on Directory objects"} }
    }
}
